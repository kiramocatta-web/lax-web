export const tenPack = `<!DOCTYPE html>
<html>
  <body style="margin:0;padding:0;background:#f0f1f5;font-family:Helvetica,Arial,sans-serif;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f1f5;">
      <tr>
        <td align="center">
          <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ede9e2;">

            <!-- Banner -->
            <tr>
              <td>
                <img
                  src="https://www.laxnlounge.com.au/emails/10pack.png"
                  width="600"
                  style="display:block;width:100%;height:auto;"
                  alt="LAX N LOUNGE"
                />
              </td>
            </tr>

            <!-- Heading -->
            <tr>
              <td style="padding:42px 32px 14px;text-align:center;">
                <h1 style="
                  margin:0;
                  font-size:36px;
                  line-height:1.1;
                  color:#2f2a24;
                  font-weight:700;
                ">
                  Your 10 Pack is Ready.
                </h1>
              </td>
            </tr>

            <!-- Intro -->
            <tr>
              <td style="padding:0 34px 30px;text-align:center;">
                <p style="
                  margin:0;
                  font-size:18px;
                  line-height:1.8;
                  color:#5a544d;
                ">
                  More recovery. Bigger savings. More consistency.
                </p>
              </td>
            </tr>

            <!-- Main Card -->
            <tr>
              <td style="padding:0 24px 28px;">
                <table
                  width="100%"
                  cellpadding="0"
                  cellspacing="0"
                  style="
                    background:#31291d;
                    border-radius:24px;
                    border:1px solid #4a4034;
                  "
                >
                  <tr>
                    <td style="padding:30px;">

                      <h2 style="
                        margin:0 0 20px;
                        font-size:28px;
                        color:#ffffff;
                        font-weight:700;
                      ">
                        What’s Included
                      </h2>

                      <p style="
                        margin:0 0 16px;
                        font-size:16px;
                        line-height:1.8;
                        color:#d8d2ca;
                      ">
                        • 10 x one-hour recovery sessions
                      </p>

                      <p style="
                        margin:0 0 16px;
                        font-size:16px;
                        line-height:1.8;
                        color:#d8d2ca;
                      ">
                        • Huge savings compared to casual bookings
                      </p>

                      <p style="
                        margin:0 0 16px;
                        font-size:16px;
                        line-height:1.8;
                        color:#d8d2ca;
                      ">
                        • Sauna, magnesium hot tub, ice bath & compression boots
                      </p>

                      <p style="
                        margin:0;
                        font-size:16px;
                        line-height:1.8;
                        color:#d8d2ca;
                      ">
                        • Built for athletes and consistent recovery routines
                      </p>

                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            <!-- CTA -->
            <tr>
              <td align="center" style="padding:8px 24px 42px;">
                <a
                  href="https://www.laxnlounge.com.au/book"
                  style="
                    display:inline-block;
                    background:#31291d;
                    color:#ffffff;
                    text-decoration:none;
                    padding:16px 38px;
                    border-radius:14px;
                    font-size:18px;
                    font-weight:700;
                  "
                >
                  Book Your Recovery
                </a>
              </td>
            </tr>

            <!-- Footer -->
            <tr>
              <td style="
                padding:28px 24px;
                background:#323232;
                text-align:center;
              ">
                <p style="
                  margin:0;
                  color:#ede9e2;
                  font-size:14px;
                  line-height:1.8;
                ">
                  88 Cook Street, Northgate QLD
                  <br />
                  © LAX N LOUNGE
                </p>
              </td>
            </tr>

          </table>
        </td>
      </tr>
    </table>
  </body>
</html>`;