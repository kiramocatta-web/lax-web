...
const STRIPE_PROMO_CODES = ["SPA", "VOLLEYBALL", "AF", "RUSH", "FINE", "RHI"];
...
function getPromoIdForCode(code: string) {
  ...
  if (code === "FINE") return process.env.STRIPE_PROMO_FINE_ID;
  if (code === "RHI") return process.env.STRIPE_PROMO_RHI_ID;
  return null;
}
